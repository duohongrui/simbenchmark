...
for(i in data_list[91:117]){
  ...
  if(data$sim_data_info$group >= 2 & "de_gene" %in% colnames(data$sim_data$row_meta)){
    ...
    if(stringr::str_starts(i, pattern = "Splat") | stringr::str_starts(i, pattern = "SCRIP")){
      ...
      saveRDS(valid_DEGs_distribution, file.path("../simulation_data/valid_DEGs_distribution", i))
    }
    ...
    if(class(error2) == "try-error"){
      ...
      saveRDS(DEGs_result, file.path("../simulation_data/DEGs_result", i))
    ...
    if(class(error3) == "try-error"){
      ...
      saveRDS(SVM_result, file.path("../simulation_data/SVM_model", i))
    ...
    saveRDS(dplyr::lst(distribution_score,
                       ...
                       Recall,
                       F1),
            file.path("../simulation_data/DEGs_evaluation", i))
